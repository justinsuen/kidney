...
public class ElementaryCyclesSearch {
	 ...
	 */
	public ElementaryCyclesSearch(boolean[][] matrix, Object[] graphNodes) throws InterruptedException {
	 ...
	 */
	public List getElementaryCycles() throws InterruptedException {
		final long startTime = System.currentTimeMillis();
		final long endTime = startTime + 90 * 1000 ; // Terminate after 2 minutes + 50ms leeway
		...
		while (System.currentTimeMillis() < endTime) {
			...
			if (sccResult != null && sccResult.getAdjList() != null) {
				...
				this.findCycles(s, s, scc, endTime);
				s++;
			} else {
				break;
			}
	 ...
	 */
	private boolean findCycles(int v, int s, Vector[] adjList, long endTime) throws InterruptedException {
		boolean f = false;
		...
		for (int i = 0; i < adjList[v].size(); i++) {
			...
			if (w == s) {
				...
				if (this.findCycles(w, s, adjList, endTime)) {
					f = true;
				}
			}
		}
